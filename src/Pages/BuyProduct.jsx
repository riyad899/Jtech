...
const BuyProduct = () => {
  ...
  const { productAPI, axiosPublic } = useAxiosPublic();
  ...
  const validateForm = () => {
    ...
    // Phone number validation for Bangladeshi numbers
    if (!formData.phone.trim()) {
      errors.phone = 'Phone number is required';
    } else {
      const phoneNumber = formData.phone.replace(/\s/g, '').replace(/-/g, '');

      // Check if it's exactly 11 digits or 14 digits with +880
      if (phoneNumber.length === 11) {
        // Format: 01XXXXXXXXX
        if (!/^01[3-9]\d{8}$/.test(phoneNumber)) {
          errors.phone = 'Please enter a valid 11-digit Bangladeshi number starting with 013-019 (e.g., 01712345678)';
        }
      } else if (phoneNumber.length === 14 && phoneNumber.startsWith('+880')) {
        // Format: +8801XXXXXXXXX
        if (!/^\+8801[3-9]\d{8}$/.test(phoneNumber)) {
          errors.phone = 'Please enter a valid Bangladeshi number with +880 (e.g., +8801712345678)';
        }
        ...
        errors.phone = 'Please enter a valid 11-digit Bangladeshi mobile number (e.g., 01712345678, +8801712345678, or 8801712345678)';
      }
    }
    ...
    if (!formData.address.trim()) {
      ...
      errors.address = 'Please provide a detailed address (at least 10 characters)';
    }
    ...
    if ((formData.paymentMethod === 'bkash' || formData.paymentMethod === 'nagad') && !formData.transactionId.trim()) {
      ...
      errors.transactionId = 'Please enter a valid transaction ID (at least 6 characters)';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send order to backend using axiosPublic
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <nav className="mb-6">
          <div className="flex items-center space-x-2 text-sm">
            ...
            <ChevronLeft className="w-4 h-4 rotate-180 text-[#0C2F4F]/40" />
            <Link to={`/product/${product._id}`} className="text-[#0D3050] hover:text-[#0C2F4F] transition-colors">
              {product.name}
            </Link>
            <ChevronLeft className="w-4 h-4 rotate-180 text-[#0C2F4F]/40" />
            <span className="text-[#0C2F4F]/60">Buy Now</span>
          </div>
          {/* Back Button */}
          <button
            onClick={() => navigate(-1)}
            ...
            <div className="bg-brand-light/80 backdrop-blur-xl border border-[#0C2F4F]/20 rounded-xl shadow-lg p-6 sticky top-32">
              ...
              <div className="flex items-start space-x-4 mb-4 p-4 bg-[#0C2F4F]/5 rounded-lg">
                <div className="w-16 h-16 bg-gradient-to-br from-[#0C2F4F]/10 to-[#0C2F4F]/20 rounded-lg flex items-center justify-center">
                  ...
                  <Package className="w-8 h-8 text-[#0C2F4F]/60" style={{display: product.image ? 'none' : 'flex'}} />
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-[#0C2F4F] mb-1">{product.name}</h3>
                  <p className="text-sm text-[#0C2F4F]/60 mb-2">{product.brand}</p>
                  <div className="flex items-center mb-2">
                    {renderStars(product.rating)}
                    <span className="ml-2 text-sm text-[#0C2F4F]/70">{product.rating}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-lg font-bold text-[#0D3050]">TK{product.price}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Quantity Selector */}
              <div className="mb-4">
                <label className="block text-sm font-medium text-[#0C2F4F]/70 mb-2">Quantity</label>
                <div className="flex items-center border border-[#0C2F4F]/20 rounded-lg">
                  <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, quantity: Math.max(1, prev.quantity - 1) }))}
                    className="px-3 py-2 text-[#0C2F4F]/60 hover:bg-[#0C2F4F]/10 transition-colors"
                  >
                    -
                  </button>
                  <span className="px-4 py-2 font-medium text-[#0C2F4F]">{formData.quantity}</span>
                  <button
                    type="button"
                        ...
                        <div className="mt-3 p-4 bg-[#0C2F4F]/5 border border-[#0C2F4F]/20 rounded-lg">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <div className="bg-white p-3 rounded-lg border border-[#0C2F4F]/10">
                                ...
                                <p className="text-sm text-[#0C2F4F]/70">Amount: TK{product.price * formData.quantity}</p>
                              </div>
                            </div>
                          ...
                          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            ...
                            <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
                              ...
                              <li>Send TK{product.price * formData.quantity} to {method.number}</li>
                ...
                >
                  {processing ? (
                    ...
                    <>
                      ...
                      <span>Place Order - TK{totalAmount}</span>
                    </>
                  )}
                </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-300 scale-100 relative">
            <div className="p-8 text-center">
              ...
              <p className="text-[#0C2F4F]/70 mb-6 leading-relaxed">
                Thank you for your order! We have received your payment details and will process your order shortly.
              ...
              <div className="bg-[#0C2F4F]/5 rounded-lg p-4 mb-6 text-left">
                ...
                <div className="space-y-1 text-sm text-[#0C2F4F]/70">
                  ...
                  <div className="flex justify-between border-t border-[#0C2F4F]/20 pt-2 mt-2">
                    ...
                    <span className="font-bold text-[#0D3050]">TK{product?.price * formData.quantity}</span>
                  </div>
                </div>
              </div>
